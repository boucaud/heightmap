...
class TextureManager {
  ...
  private async load(url: string) {
    return new Promise<Texture>((resolve, reject) => {
      this.loader.load(url, resolve, () => {}, reject);
    });
  }
  ...
  async loadAllTextures() {
    return Promise.all(
      [
        this.load(colormap).then((texture: Texture) => this.colorMapTexture = texture),
        this.load(heightmap).then((texture: Texture) => this.heightMapTexture = texture),
      ]
    );
  }
}
